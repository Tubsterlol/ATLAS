...
from aerospace.aircraft.flight_conditions import mach_number, reynolds_number
from aerospace.aircraft.lift import aircraft_lift
from aerospace.aircraft.performance import aircraft_thrust_to_weight, stall_speed
from aerospace.physics.constants import SEA_LEVEL_AIR_DENSITY
...
class AircraftSimulation(BaseSimulation):
    def __init__(self, aircraft, velocity_ms: float, timestep_s: float = 1.0):
    ...
    def step(self):
        ...
        twr = aircraft_thrust_to_weight(
            thrust_n=self.aircraft.thrust_n, mass_kg=self.aircraft.mass_kg
        )
        ...
        self.velocity_ms += acceleration * self.state.timestep_s

        self.velocity_ms = min(self.velocity_ms, self.aircraft.max_speed_ms)

        mach = mach_number(velocity_ms=self.velocity_ms, temperature_k=288.15)
